...
function useUpdateHoverEffect() {
  const removeHoverEffect = () => {
    document
      .querySelectorAll('.list-cell-sortable-hover')
      .forEach(elem => elem.classList.remove('list-cell-sortable-hover'))
  }
...
function swapCustomFieldPosition(
  customFields: Field[],
  draggedItem: number,
  index: number
) {
...
export default function CustomFieldSortableCell({
  children,
  width,
  index
}: {
  ...
  const updateAllCustomFields = useProjectCustomFieldStore(
  ...
  const handleDragEnd = (e: DragEvent<HTMLDivElement>) => {
    ...
    if (draggedItem) {
      removeHoverEffect()
      const updatedCustomfields = swapCustomFieldPosition(
        customFields,
        draggedItem,
        oldIndex
      )
