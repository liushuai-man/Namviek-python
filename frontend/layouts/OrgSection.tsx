      ...
      <div className="w-9 h-9 relative p-1 rounded-md bg-zinc-100 dark:bg-gray-800 dark:border-gray-700">
        ...
        {org.cover ? (
          <img
            ...
            src={org.cover}
            alt="Cover organization"
          />
        ) : (
          <Loading.Absolute className="rounded-md" title="" />
        )}
      </div>
      <div className="flex flex-col org-section-info">
        <span className="text-gray-700 dark:text-gray-400 text-sm truncate">
          {org.name ? (
            org.name
          ) : (
            <span className="h-2 bg-slate-200 dark:bg-slate-700 rounded animate-pulse text-transparent">
              No title
            </span>
          )}
        </span>
        <span className="text-[11px] text-gray-400 dark:text-gray-500">
          There {len > 2 ? 'are' : 'is'} {len} member{len > 1 ? 's' : ''}
        </span>
      </div>
    </div>
  )
...
function OrgPopMenu({ orgName }: { orgName: string }) {
  ...
  return (
    <div className="org-section-popup">
      <Popover
        triggerBy={
          <div className="w-7 h-7 rounded-md p-1 group cursor-pointer  dark:border-gray-700 hover:bg-zinc-100 dark:hover:bg-gray-800 flex items-center justify-center">
...
export default function OrgSection() {
  ...
  return (
    <section className="nav-org-section border-b dark:border-gray-800 px-3 pt-[20px] pb-[21px]">
